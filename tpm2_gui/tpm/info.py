...
class Traversable:  # pylint: disable=too-few-public-methods
    """Attributes are traversable recurively."""
...
class Capabilities(Traversable):
    ...
    def _get_cap_data(self, description):
        return next(cap for cap in self.data if cap["description"] == description)["info"]["data"]
...
def int_list_as_string(int_list):
    """Cast integers to bytes and decode as string."""
    string = b"".join(integer.to_bytes(4, byteorder="big") for integer in int_list).decode("utf-8")
...
class TPMInfo(Traversable):
    ...
    def manufacturer(self):
        ...
        return int_list_as_string([self.capabilities.properties_fixed.manufacturer])
    ...
    def firmware_version(self):
        ...
        return defaultdict(lambda: f"{ver1:x}.{ver2:x}", {"IBM.SW TPM": f"{ver1:x}.{ver2:x}"})[key]
